...
| AV-07 | Reviewed | 2026-08-19 | Student accepted the equipment/safety evidence and ID-only stationary gate. Metric robot-relative validation remains deferred until actual VisionPortal resolution and matching Logitech C920 calibration are verified. Java 17 builds and `git diff --check` passed. |
| AV-08 | Reviewed | 2026-08-19 | Student accepted supervised ID-only validation with drive motors disconnected. C920 640-by-480 detection, loss/search, reacquisition, processing timestamp/age, stability, stop, preview closure, and clean resource reacquisition passed. Metric validation remains deferred pending matching calibration. |
