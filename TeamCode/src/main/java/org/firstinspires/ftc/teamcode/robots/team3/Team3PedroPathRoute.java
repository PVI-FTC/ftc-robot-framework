...
/** Team 3 paths that can be selected from the Pedro autonomous menu. */
public enum Team3PedroPathRoute {
    ...
    Team3PedroPathRoute(String displayName, double startXInches, double startYInches,
    ...
    PathChain build(Follower follower) {
        switch (this) {
            case LP11_PILOT:
                return Team3PedroPilotPath.build(follower);
            case DECODE_ROUTE:
                return Team3PedroDecodePath.build(follower);
            case CURVING_TEST:
                return Team3PedroCurvingTestPath.build(follower);
            case ILLEGAL_PATH:
                return Team3PedroIllegalPath.build(follower);
            default:
                throw new IllegalStateException("No Team 3 Pedro path builder exists for " + this);
        }
    }
}
